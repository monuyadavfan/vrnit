...
def create_app():
    ...
    def keys_page():
        ...
        used_rows = "".join([
            f"<tr><td><span class='tag'>{k}</span></td><td>{v.get('days','?')}d</td>"
            f"<td><span class='tag'>{v.get('used_by','?')}</span></td>"
            f"<td>{v.get('used_at','')[:10]}</td></tr>"
            for k,v in used[-20:]
        ])
        c = f"""
        ...
        {used_rows or '<tr><td colspan=4 style=color:#555;text-align:center>None</td></tr>'}</table></div>"""
